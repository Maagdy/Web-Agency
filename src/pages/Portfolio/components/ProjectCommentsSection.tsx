...
import { useProjectComments } from "../../../hooks/PortfolioPageHooks/useProjectComments";
...
function ProjectCommentsSection({ projectId }: { projectId: number }) {
  const { comments, loading, error, refetch } = useProjectComments(
    projectId,
    true
  );
  ...
  const handleAddComment = async (data: CommentModalData) => {
    ...
    try {
      ...
      if (error) {
        console.error("âŒ Error saving comment:", error.message);
      } else {
        console.log("âœ… Comment saved successfully!", insertedData);
